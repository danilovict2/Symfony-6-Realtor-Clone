<template>
    <div class="max-w-6xl mx-auto px-3">
        <h1 class="text-3xl text-center mt-6 font-bold mb-6">
            Places for {{ props.category }}
        </h1>
...
function fetchListings() {
    ...
    axios.get('/listing/api/fetch/' + offset)
        .then(response => {
            listings.value = response.data.listings.filter((listing) => {return listing.type === props.category});
